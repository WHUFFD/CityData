...
crfo<-function(){
    browser()
    rltnet=data.table(read.table("net.csv",header=FALSE,sep=","))
    x=data.table(t(read.table("userdefine.csv",header=FALSE,sep=",")))$V1
    ...
    write.table(rltn,"/usr/share/nginx/html/processfile/file/roads.csv",sep=",",row.names = FALSE,col.names = FALSE)
}
